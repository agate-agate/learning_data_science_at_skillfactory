...
def game_core_v4(secret_number, show_path=False):
    ...
    На выходе - словарь со статистикой Игрока за одну Игру (партию)
    Поля словаря
        is_win -- булева - победил ли Игрок? False если Сдался.
        attempt_count -- число - сколько Попыток за Игру (партию) сделал
                         Игрок
        path -- список чисел - (опциональное поле - оно не показывается
                если аргумент show_path равен False) - все
                ПредположенныеЧисла которые обдумывал Игрок за Игру
                (партию); в некоторых случаях тут может быть больше
                чисел чем было сделано Попыток - потому что Игрок не все
                числа решился Огласить вслух.
    ...
    2. Игрок - придумывает и называет вслух одно ПредположенноеЧисло из
    интервала [1;100] и Вода сразу честно оглашает ему один из трёх
    исходов, который подходит под ситуацию:
    ...
    4. Если Игрок Победил, то участники игры считают количество Попыток
    и чем оно меньше - тем большим поводом для Гордости Игроку оно
    служит :)
    ...
    проверить этот алгоритм на жизнеспособность.
